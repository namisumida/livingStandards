function init() {
  ...
  function setup() {
    ...
    svg_lifeExpectancy_timeline.selectAll(".compGroup")
                               ...
                               .attr("x", function(d) {
                                 var oldScaled = runDotScale("lifeExpectancy", d.lifeExpectancy_old, "ldc");
                                 var latestScaled = runDotScale("lifeExpectancy", d.lifeExpectancy_latest, "ldc");
                                 // if it fits
                                 if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) {
                                   // if older is greater than latest
                                   if (d.lifeExpectancy_old > d.lifeExpectancy_latest){ return latestScaled - 10; }
                                   else { return latestScaled - 10; }
                                 }
                                 // if it doesn't fit
                                 else { return oldScaled - 10; }
                               })
                               .style("text-anchor", function(d) {
                                 var oldScaled = runDotScale("lifeExpectancy", d.lifeExpectancy_old, "ldc");
                                 var latestScaled = runDotScale("lifeExpectancy", d.lifeExpectancy_latest, "ldc");
                                 if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) {
                                   if (d.lifeExpectancy_old > d.lifeExpectancy_latest) { return "end"; }
                                   else { return "end"; }
                                 }
                                 // doesn't fit
                                 else { return "end"; }
    ...
    svg_lifeExpectancy_timeline.selectAll(".compGroup")
                               .on("mouseover", function(d) {
                                 ...
                                 currGroup.append("text")
                                          ...
                                          .attr("x", function(d) {
                                            // if it fits
                                            if (parseFloat(currGroup.select(".timelineRect").node().getBBox().width) >= currGroup.select(".timelineLabels").node().getComputedTextLength()+ 20) {
                                              return parseFloat(currGroup.select(".timelineDots_old").attr("cx"))-10;
                                            }
                                            else { return parseFloat(currGroup.select(".timelineDots_old").attr("cx")) - parseFloat(currGroup.select(".timelineLabels").node().getBBox().width) - 30; }
                                          })
                                          .style("text-anchor", function() {
                                            if (parseFloat(currGroup.select(".timelineRect").node().getBBox().width) >= currGroup.select(".timelineLabels").node().getComputedTextLength()+ 20) { return "end"; }
                                            else { return "start"; }
                                          })
                                ...
                                currGroup.append("text")
                                         ...
                                         .attr("x", function(d) { return parseFloat(currGroup.select(".timelineDots").attr("cx"))+10; })
                                         .attr("y", parseFloat(currGroup.select(".timelineLabels").attr("y")))
    ...
    svg_lifeExpectancy_comp.selectAll(".compAvgGroup")
                           ...
                           .attr("x", function(d) {
                             if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) { return runDotScale("lifeExpectancy", d.old, "ldc") + 15; }
                             else { return runDotScale("lifeExpectancy", d.old, "ldc") - 15; }
                           })
                           .attr("y", function(d,i) { return 29 + i*30; })
                           .style("text-anchor", function(d) {
                             if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) { return "start"; }
                             else { return "end"; }
                           })
    ...
    svg_lifeExpectancy_comp.selectAll(".compAvgGroup")
                           ...
                           .attr("x", function(d,i) {
                             if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) { return runDotScale("lifeExpectancy", d.old, "ldc")-15; }
                               ...
                               if (i==0) { return runDotScale("lifeExpectancy", d.old, "ldc")+10; }
                               else { return runDotScale("lifeExpectancy", d.old, "ldc")+15; }
                             }
                           })
                           .attr("y", function(d,i) { return 29 + i*30; })
                           .style("text-anchor", function(d) {
                             if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) { return "end"; }
                             else { return "start"; }
                           })
                           .style("fill", yellow);
    svg_lifeExpectancy_comp.selectAll(".compAvgGroup")
                           ...
                           .attr("x", function(d) { return runDotScale("lifeExpectancy", d.latest, "ldc")+15; })
                           .attr("y", function(d,i) { return 29 + i*30; })
                           .style("text-anchor", "start")
                           .style("fill", blue);
    ...
    svg_lifeExpectancy_comp.selectAll(".compGroup")
                           ...
                           .attr("class", "timelineRect")
                           .attr("y", function(d,i) {
                             if (w>=500) { return 69 + (i+1)*25; }
                             else { return 70 + (i+1)*25; }
                           })
                           .attr("height", function(d,i) {
                             if (w>=500) { return 12; }
                             else { return 10; }
    ...
    svg_lifeExpectancy_comp.selectAll(".compGroup")
                           ...
                           .attr("x", function(d) {
                             ...
                             if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) {
                               // if older is greater than latest
                               if (d.lifeExpectancy_old > d.lifeExpectancy_latest){ return oldScaled - 10; }
                               else { return latestScaled - 10; }
                             }
                             // if it doesn't fit
                             else { return oldScaled - 10; }
    ...
    svg_otherMetrics.selectAll(".compAvgGroup")
                     ...
                     .attr("y", function(d,i) {
                       if (w>=500) {
                         if (i==2) { return 18 + (i*30); }
                         else { return 19 + (i*30); }
                       }
                       else {
                         if (i==2) { return 19 + (i*30); }
                         else { return 20 + (i*30); }
                       }
                     })
                     .attr("height", function(d,i) {
    ...
    svg_otherMetrics.selectAll(".compAvgGroup")
                     ...
                     .attr("id", "axisLabelLatest")
                     .text(function(d) { return Math.round(d.latest); })
                     .attr("x", function(d) { return runDotScale("undernourished", d.latest, "ldc")-15; })
                     .attr("y", function(d,i) { return 29 + i*30; })
    ...
    svg_otherMetrics.selectAll(".compGroup")
                     ...
                     .attr("y", function(d,i) {
                       if (w>=500) { return 79 + (i+1)*25; }
                       else { return 80 + (i+1)*25; }
                     })
                     .attr("height", function() {
                       if (w>=500) { return 12; }
                       else { return 10; }
    ...
    svg_otherMetrics.selectAll(".compGroup")
                     ...
                     .text(function(d) { return d.countryName; })
                     .attr("x", function(d) {
                       var oldScaled = runDotScale("undernourished", d.undernourished_old, "ldc");
                       var latestScaled = runDotScale("undernourished", d.undernourished_latest, "ldc");
                       if (Math.abs(latestScaled-oldScaled) >= this.getComputedTextLength()+ 20) { // if it fits
                         // if older is greater than latest
                         if (d.undernourished_old > d.undernourished_latest){ return latestScaled + 10; }
                         else { return latestScaled - 10; }
                       }
                       else { // if it doesn't fit
                         if (d.undernourished_old > d.undernourished_latest) { return oldScaled + 10; }
                         else { return latestScaled + 10; }
                       }
                     })
                     .style("text-anchor", function(d) {
                       var oldScaled = runDotScale("undernourished", d.undernourished_old, "ldc");
                       var latestScaled = runDotScale("undernourished", d.undernourished_latest, "ldc");
                       if (Math.abs(latestScaled-oldScaled) >= this.getComputedTextLength()+ 20) {
                         if (d.undernourished_old > d.undernourished_latest) { return "start"; }
                         else { return "end"; }
                       }
                       else { return "start"; } // doesn't fit
                     })
    ...
    svg_otherMetrics.selectAll(".compGroup")
                     .on("mouseover", function(d) {
                       ...
                       currGroup.append("text")
                                ...
                                .attr("x", function(d) {
                                  ...
                                  if (parseFloat(currGroup.select(".timelineRect").node().getBBox().width) >= currGroup.select(".timelineLabels").node().getComputedTextLength()+ 20) {
                                    ...
                                    if (d.undernourished_old > d.undernourished_latest) { return oldScaled + parseFloat(currGroup.select(".timelineLabels").node().getBBox().width) + 13; }
                                    else { return oldScaled - 10; }
                                  }
                                })
                                .attr("y", parseFloat(currGroup.select(".timelineLabels").attr("y")))
                                .style("text-anchor", function(d) {
                                  if (parseFloat(currGroup.select(".timelineRect").node().getBBox().width) >= currGroup.select(".timelineLabels").node().getComputedTextLength()+ 20 & d.undernourished_old > d.undernourished_latest) { return "start"; }
                                  else {
                                    if (d.undernourished_old > d.undernourished_latest) { return "start"; }
                                    else { return "end"; }
                                  }
                      ...
                      currGroup.append("text")
                               ...
                               .attr("x", function(d) {
                                 // if country name fits in rectangle
                                 if (parseFloat(currGroup.select(".timelineRect").node().getBBox().width) >= currGroup.select(".timelineLabels").node().getComputedTextLength()+ 20) {
                                   if (d.undernourished_old > d.undernourished_latest) { return latestScaled - 10; }
                                   else { return latestScaled + 10; }
                                 }
                                   ...
                                   if (d.undernourished_old > d.undernourished_latest) { return latestScaled - 10; }
                                   else { return latestScaled + 13 + parseFloat(currGroup.select(".timelineLabels").node().getBBox().width); }
                                 }
                               })
                               .attr("y", parseFloat(currGroup.select(".timelineLabels").attr("y")))
                               .style("text-anchor", function(d) {
  ...
  function resize() {
    ...
    updateView("lifeExpectancy", svg_lifeExpectancy_timeline, "resize", 1);
    // Life expectancy US vs LDC svg
    updateView("lifeExpectancy", svg_lifeExpectancy_comp, "resize", 2);
    // Other metrics svg
    var variableOld = currOtherMetric + "_old";
    var variableLatest = currOtherMetric + "_latest";
    svg_otherMetrics.selectAll("#axisLabelYear")
                    .attr("x", function(d,i) {
                      if (i==0) { return runDotScale(currOtherMetric, dataset_compAvg.filter(function(d) { return d.country=="world"})[0].old, "ldc"); }
                      else { return runDotScale(currOtherMetric, dataset_compAvg.filter(function(d) { return d.country=="world"})[0].latest, "ldc"); }
                    });
    svg_otherMetrics.selectAll(".lineMarkers_timeline")
                     .attr("x1", function(d) { return runDotScale(currOtherMetric, d, "ldc"); })
                     .attr("x2", function(d) { return runDotScale(currOtherMetric, d, "ldc"); })
    svg_otherMetrics.selectAll(".lineMarkerLabels")
                     .attr("x", function(d) { return runDotScale(currOtherMetric, d, "ldc"); })
                     .attr("y", h_svgOther-5)
                     .text("US avg")
                     .call(wrap, 60);
    svg_otherMetrics.selectAll(".compAvgGroup").select(".timelineRect")
                     .attr("x", function(d) {
                       if ((currOtherMetric=="undernourished" | currOtherMetric=="under5" | currOtherMetric=="maternal")) { return runDotScale(currOtherMetric, d.latest, "ldc"); }
                       else { return runDotScale(currOtherMetric, d.old, "ldc"); }
                     })
                     .attr("width", function(d) { return Math.abs(runDotScale(currOtherMetric, d.old, "ldc") - runDotScale(currOtherMetric, d.latest, "ldc")); })
                     .attr("y", function(d,i) {
                       if (w>=500) {
                         if (i==2) { return 18 + (i*30); }
                         else { return 19 + (i*30); }
    ...
    svg_otherMetrics.selectAll(".compAvgGroup").select(".timelineDots_old")
                    .attr("cx", function(d) { return runDotScale(currOtherMetric, d.old, "ldc"); })
                    .attr("r", function(d,i) {
                      if (i==2) {
                        if (w>=500) { return 10; }
                        else { return 7; }
    ...
    svg_otherMetrics.selectAll(".compAvgGroup").select(".timelineDots")
                    .attr("cx", function(d) { return runDotScale(currOtherMetric, d.latest, "ldc"); })
                    .attr("r", function(d,i) {
                      if (i==2) {
                        if (w>=500) { return 10; }
                        else { return 7; }
    ...
    svg_otherMetrics.selectAll(".compAvgGroup").select(".timelineLabels")
                    .attr("x", function(d,i) {
                       if ((currOtherMetric=="undernourished" | currOtherMetric=="under5" | currOtherMetric=="maternal") & i!=1) {
                         if (d.old>99) { return runDotScale(currOtherMetric, d.old, "ldc") + 45; }
                         else { return runDotScale(currOtherMetric, d.old, "ldc") + 35; }
                       }
                       else { return runDotScale(currOtherMetric, d.latest, "ldc") + 15; }
                     });
    svg_otherMetrics.selectAll(".compAvgGroup").select("#axisLabelOld")
                     .attr("x", function(d) {
                       if (currOtherMetric=="undernourished" | currOtherMetric=="under5" | currOtherMetric=="maternal") { return runDotScale(currOtherMetric, d.old, "ldc")+15; }
                       else { return runDotScale(currOtherMetric, d.old, "ldc")-15; }
                     });
    svg_otherMetrics.selectAll(".compAvgGroup").select("#axisLabelLatest")
                     .attr("x", function(d) {
                       if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) { return runDotScale(currOtherMetric, d.latest, "ldc")-15; }
                       else { return runDotScale(currOtherMetric, d.latest, "ldc")-15; }
                     });
    // if displaying individual countries
    if (d3.select("#button-otherRandomize").style("display")=="none") {
       svg_otherMetrics.selectAll(".compGroup").select(".timelineRect")
                        .attr("x", function(d) { return runDotScale(currOtherMetric, d[variableOld], "ldc"); });
       svg_otherMetrics.selectAll(".compGroup").select(".timelineDots")
                        .attr("r", function() {
                          if (w>=500) { return 7; }
                          else { return 5; }
                        })
                        .attr("cx", function(d) { return runDotScale(currOtherMetric, d[variableOld], "ldc"); });
       svg_otherMetrics.selectAll(".compGroup").select(".timelineDots_old")
                        .attr("r", function() {
                          if (w>=500) { return 7; }
                          else { return 5; }
                        })
                        .attr("cx", runDotScale(currOtherMetric, 35, "ldc"));
       svg_otherMetrics.selectAll(".compGroup").select(".timelineLabels")
                        .attr("x", function(d) {
                          var oldScaled = runDotScale(currOtherMetric, d[variableOld], "ldc");
                          var latestScaled = runDotScale(currOtherMetric, d[variableLatest], "ldc");
                          // if it fits
                          if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) {
                            // if older is greater than latest
                            if (d[variableOld] > d[variableLatest]){ return oldScaled - 10; }
                            else { return latestScaled - 10; }
                          }
                          // if it doesn't fit
                          else { return oldScaled - 10; }
    ...
    } // end if button-otherRandomize is not displayed
    else { // if displayed
       svg_otherMetrics.selectAll(".compGroup").select(".timelineRect")
                        .attr("x", function(d) {
                          if (currOtherMetric=="undernourished" | currOtherMetric=="under5" | currOtherMetric=="maternal") {
                            if (d[variableOld] > d[variableLatest]) { return runDotScale(currOtherMetric, d[variableLatest], "ldc"); }
                            else { return runDotScale(currOtherMetric, d[variableOld], "ldc"); }
                          }
                          else {
                            if (d[variableOld] > d[variableLatest]) { return runDotScale(currOtherMetric, d[variableLatest], "ldc"); }
                            else { return runDotScale(currOtherMetric, d[variableOld], "ldc"); }
                        ...
                        .attr("width", function(d) { return Math.abs(runDotScale(currOtherMetric, d[variableLatest], "ldc") - runDotScale(currOtherMetric, d[variableOld], "ldc")); })
                        .attr("y", function(d,i) {
                          if (w>=500) { return 79 + (i+1)*25; }
                          else { return 80 + (i+1)*25; }
                        })
       ...
       svg_otherMetrics.selectAll(".compGroup").select(".timelineDots")
                       .attr("cx", function(d) { return runDotScale(currOtherMetric, d[variableLatest], "ldc"); })
                       .attr("r", function() {
                         if (w>=500) { return 7; }
                         else { return 5; }
                       });
       svg_otherMetrics.selectAll(".compGroup").select(".timelineLabels")
                        .attr("x", function(d) {
                          if (currOtherMetric=="lifeExpectancy" | currOtherMetric=="undernourished" | currOtherMetric=="under5" | currOtherMetric=="maternal") {
                            if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20)
                             if (d[variableOld] > d[variableLatest]){ return runDotScale(currOtherMetric, d[variableOld], "ldc") - 10; }
                             else { return runDotScale(currOtherMetric, d[variableOld], "ldc") + 10; }
                            else if (d[variableOld] > d[variableLatest]) { return runDotScale(currOtherMetric, d[variableOld], "ldc") + 10; }
                            else { return runDotScale(currOtherMetric, d[variableOld], "ldc") - 10; } // if increased over time
                          }
                          else {
                            // if it fits
                            if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20)
                            // if older is greater than latest
                             if (d[variableOld] > d[variableLatest]){ return runDotScale(currOtherMetric, d[variableOld], "ldc") - 10; }
                             else { return runDotScale(currOtherMetric, d[variableLatest], "ldc") - 10; }
  ...
  function updateView(metric, svg, type, svgID) {
    ...
    if (svgID==1) { // world view
      ...
      if (type == "sortChange") {
        ...
        svg.selectAll(".compAvgGroup").select(".timelineRect")
            .attr("x", function(d) { return runDotScale("lifeExpectancy", d.old, "ldc"); })
            .attr("width", function(d) { return runDotScale("lifeExpectancy", d.latest, "ldc") - runDotScale("lifeExpectancy", d.old, "ldc"); })
            .attr("y", function(d,i) {
              if (w>=500) {
        ...
        svg.selectAll(".compAvgGroup").select(".timelineDots_old")
            .attr("cx", function(d) { return runDotScale("lifeExpectancy", d.old, "ldc"); })
            .attr("r", function(d,i) {
              if (i==1) {
                if (w>=500) { return 10; }
        ...
        svg.selectAll(".compAvgGroup").select(".timelineDots")
            .attr("cx", function(d) { return runDotScale("lifeExpectancy", d.latest, "ldc"); })
            .attr("r", function(d,i) {
              if (i==1) {
                if (w>=500) { return 10; }
        ...
        svg.selectAll(".compAvgGroup").select(".timelineLabels")
            .attr("x", function(d) {
              if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) { return runDotScale("lifeExpectancy", d.old, "ldc") + 15; }
              else { return runDotScale("lifeExpectancy", d.old, "ldc") - 15; }
            })
        ...
        svg.selectAll(".compAvgGroup").select("#axisLabelOld")
            .attr("x", function(d,i) {
              if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) { return runDotScale("lifeExpectancy", d.old, "ldc")-15; }
              else {
                if (i==0) { return runDotScale("lifeExpectancy", d.old, "ldc")+10; }
        ...
        svg.selectAll(".compAvgGroup").select("#axisLabelLatest")
            .attr("x", function(d) { return runDotScale("lifeExpectancy", d.latest, "ldc")+15; })
            .style("text-anchor", "start")
        svg.selectAll(".compGroup").select(".timelineRect")
           .attr("y", function(d,i) {
    ...
    svg.selectAll(".compGroup")
       ...
       .text(function(d) { return d.countryName; })
       .attr("x", function(d) {
          var oldScaled = runDotScale(metric, d[variableOld], scaleType);
          var latestScaled = runDotScale(metric, d[variableLatest], scaleType);
          if (metric=="undernourished" | metric=="under5" | metric=="maternal") {
            if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) { // if it fits
              // if older is greater than latest
              if (d[variableOld] > d[variableLatest]){ return latestScaled + 10; }
              else { return latestScaled - 10; }
            }
            // if it doesn't fit
            else {
          ...
          else if (metric=="electricity" | metric=="sanitation") { // metric is access
            if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) { // if it fits
              // if older is greater than latest
              if (d[variableOld] > d[variableLatest]){ return latestScaled + 10; }
              else { return latestScaled - 10; }
       ...
       .style("text-anchor", function(d) {
          ...
          else if (metric=="electricity" | metric=="sanitation") { // access vars
            if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) {
              if (d[variableOld] > d[variableLatest]) { return "start"; }
              else { return "end"; }
            }
  ...
  function changeMetric(metric) { // change to a different metric
    ...
    svg_otherMetrics.selectAll(".compAvgGroup").select("#axisLabelLatest")
                     .text(function(d) { return Math.round(d.latest); })
                     .attr("x", function(d) {
                       if (parseFloat(d3.select(this.parentNode).select(".timelineRect").node().getBBox().width) >= this.getComputedTextLength()+ 20) { return runDotScale(metric, d.latest, "ldc")-15; }
                       else { return runDotScale(metric, d.latest, "ldc")-15; }
                     })
                     .style("text-anchor", "end");
    ...
    svg_otherMetrics.selectAll(".compGroup")
                     .on("mouseover", function(d) {
                       ...
                       currGroup.append("text")
                                ...
                                .attr("x", function(d) {
                                  ...
                                  if (parseFloat(currGroup.select(".timelineRect").node().getBBox().width) >= currGroup.select(".timelineLabels").node().getComputedTextLength()+ 20) {
                                    ...
                                    if (d[variableOld] > d[variableLatest]) { return oldScaled + parseFloat(currGroup.select(".timelineLabels").node().getBBox().width) + 13; }
                                    else { return oldScaled - 10; }
                                  }
                                })
                                .attr("y", parseFloat(currGroup.select(".timelineLabels").attr("y")))
                      ...
                      currGroup.append("text")
                               ...
                               .attr("x", function(d) {
                                 if (metric=="undernourished" | metric=="under5" | metric=="maternal") {
                                   if (parseFloat(currGroup.select(".timelineRect").node().getBBox().width) >= currGroup.select(".timelineLabels").node().getComputedTextLength()+ 20) { // if it fits
                                     if (d[variableOld] < d[variableLatest]) { return latestScaled + 10; }
                                     else { return latestScaled-10; }
                                   }
                                   else {
                                     if (d[variableOld] < d[variableLatest]) { return latestScaled + 13 + currGroup.select(".timelineLabels").node().getComputedTextLength(); }
                                     else { return latestScaled-10; }
                                   }
                                 }
                                 else {
                                   ...
                                   // if it doesn't fit
                                     ...
                                     if (d[variableOld] < d[variableLatest]) { return latestScaled + parseFloat(currGroup.select(".timelineLabels").node().getBBox().width) + 13; }
                                     else { return latestScaled - 10; }
                                   }
                                 }
                               })
